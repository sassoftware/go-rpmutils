...
func Vercmp(first, second string) int {
	...
	for first != "" || second != "" {
		...
		m1_head, first = m1[1], m1[2]
		m2_head, second = m2[1], m2[2]
		...
		if isnum {
			...
			m1_head = strings.TrimLeft(m1_head, "0")
			m2_head = strings.TrimLeft(m2_head, "0")
